...
int main(void){
	...
	// Interrupt setuü
	GPIOIntDisable(GPIO_PORTA_BASE, buttonUpPin|buttonDownPin);        // Disable interrupt for PF4 (in case it was enabled)
	GPIOIntClear(GPIO_PORTA_BASE, buttonUpPin|buttonDownPin);      // Clear pending interrupts for PF4
	GPIOIntRegister(GPIO_PORTA_BASE, portAISR);     // Register our handler function for port F
	GPIOIntTypeSet(GPIO_PORTA_BASE, buttonUpPin|buttonDownPin,GPIO_FALLING_EDGE);             // Configure PF4 for falling edge trigger
	GPIOIntEnable(GPIO_PORTA_BASE, buttonUpPin|buttonDownPin);     // Enable interrupt for PF4
